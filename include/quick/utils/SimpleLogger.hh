...
inline void info(
    ...
    Args&&... args) {
  impl_logger().info(loc.function_name(),
...
inline void debug(
    const std::source_location& loc = std::source_location::current(),
    Args&&... args) {
  impl_logger().debug(loc.function_name(), static_cast<int>(loc.line()),
                      std::forward<Args>(args)...);
}
...
inline void error(
    const std::source_location& loc = std::source_location::current(),
    Args&&... args) {
  impl_logger().error(loc.function_name(), static_cast<int>(loc.line()),
                      std::forward<Args>(args)...);
}
...
inline void warn(
    const std::source_location& loc = std::source_location::current(),
    Args&&... args) {
  impl_logger().warn(loc.function_name(), static_cast<int>(loc.line()),
                     std::forward<Args>(args)...);
}

// Note: these macros use std::source_location to capture caller info.
#define LOG_INFO_S(...) \
  ::quick::utils::info(std::source_location::current(), __VA_ARGS__)
#define LOG_DEBUG_S(...) \
  ::quick::utils::debug(std::source_location::current(), __VA_ARGS__)
#define LOG_ERROR_S(...) \
  ::quick::utils::error(std::source_location::current(), __VA_ARGS__)
#define LOG_WARN_S(...) \
  ::quick::util::warn(std::source_location::current(), __VA_ARGS__)

...
class TcpServer : public Tcp {
 ...
 public:
  ...
  __always_inline [[gnu::hot]] auto send(SocketRAII& client, const void* buf,
                                         size_t len)
  ...
  __always_inline [[gnu::hot]] auto recv(SocketRAII& client, void* buf,
                                         size_t len)
      -> std::expected<std::uint64_t, quick::error::TcpError> {
...
inline TcpServer::TcpServer(const std::string& ip, uint16_t port) noexcept
    : Tcp{ip, port} {}

inline TcpServer::TcpServer(std::string&& ip, uint16_t port)
...
inline auto TcpServer::accept_client()
    -> std::expected<SocketRAII, quick::error::TcpError> {
  quick::utils::Timer timer{"accept_client"};
  int client_fd = ::accept(m_sock, nullptr, nullptr);
  if (client_fd < 0) return std::unexpected(quick::error::TcpError::BAD_SOCKET);

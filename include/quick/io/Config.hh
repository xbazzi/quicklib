...
class Config {
 private:
  ...
  static inline quick::utils::Logger<Config>& m_logger{
      quick::utils::Logger<Config>::get_instance("Config")};
 ...
 public:
  ...
  Config(const std::filesystem::path& path)
      : m_config_path{path}, m_parser{path} {}
  ~Config() = default;
  ...
  bool parse_config() noexcept {
    return m_parser.load()
        ...
        .or_else([](error::TomlParserError error)
                     -> std::expected<bool, error::TomlParserError> {

...
class TcpClient : public Tcp {
 public:
  ...
  TcpClient() = default;
  explicit TcpClient(const std::string& ip, const std::uint16_t port)
      : Tcp{ip, port} {}
  explicit TcpClient(std::string&& ip, const std::uint16_t port)
  ...
  inline std::expected<void, quick::error::TcpError> connect_to_server() {
    ...
    if (::connect(m_sock, reinterpret_cast<sockaddr*>(&server),
                  sizeof(server)) < 0)
      return std::unexpected(quick::error::TcpError::CONNECT_FAIL);
    return {};
  }

  [[gnu::hot]] inline std::expected<std::uint64_t, quick::error::TcpError> send(
  ...
  [[gnu::hot]] inline std::expected<std::uint64_t, quick::error::TcpError> recv(
      void* buf, size_t len) {

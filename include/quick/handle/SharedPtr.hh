...
struct control_block : private Deleter {
  ...
  control_block(const Deleter& custom_deleter = Deleter{}) noexcept(
      std::is_nothrow_copy_constructible_v<Deleter>)
      : Deleter{custom_deleter} {}

  void init(T* ptr) noexcept { p_obj = ptr; }

  void add_ref() noexcept { m_count.fetch_add(1, std::memory_order_acq_rel); }
...
class shared_ptr {
 ...
 public:
  ...
  shared_ptr(const shared_ptr& other) noexcept
      : p_obj{other.p_obj}, p_cb{other.p_cb} {
    if (p_cb) {
      p_cb->add_ref();
    }
  }
  ...
  size_t use_count() const { return p_cb ? p_cb->use_count() : 0; }

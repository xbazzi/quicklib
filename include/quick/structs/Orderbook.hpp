 ...
 public:
  ...
  void insert_order(const Order& order) {
    ...
    if (order.is_buy()) {
      ...
      auto pos = std::ranges::lower_bound(
          bids_, order.get_level(), std::less<Price>(), &Order::get_level);
  ...
  Trades AddOrder(const Order& incoming) {
    ...
    while (!opposite_side.empty() and remaining > 0) {
      ...
      Price trade_price =
          incoming.is_buy() ? best.get_level() : incoming.get_level();
      ...
      trades.emplace_back(Trade{bid_order_id, ask_order_id, aggressor_id,
                                aggressor_is_buy, trade_price, trade_size});
...
int main() {
  ...
  // Quantity target_qty      = magic;
  std::size_t n_orders = magic * 2;

  std::ranges::generate_n(std::back_inserter(orders), n_orders, [&]() mutable {
  ...
  std::ranges::for_each(orders,
                        [](const Order& o) { std::cout << o << std::endl; });
  Orderbook orderbook;
  Trades trades;
  ...
  std::ranges::for_each(trades, [&](const Trade& t) {

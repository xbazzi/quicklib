...
class vector {
 ...
 public:
  ...
  void push_back(T&& element) {
    ...
    if (m_size >= m_cap) _grow_capacity(m_cap * 3);
  ...
  const Element& at(std::size_t index) const {
    if (index >= m_size) throw std::out_of_range("u tripping");
    return *(static_cast<Element*>(p_arr + index));
  }

  std::size_t get_size() const { return m_size; }

  std::size_t get_capacity() const { return m_cap; }

  void shrink_to_fit() {
    if (m_cap > m_size) _grow_capacity(m_size);
  }

  void resize(uint64_t new_cap) { m_cap = new_cap; }
